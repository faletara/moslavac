...
/** Absolute image URL for a match (home/away team crest), or null to fall back to the generated OG image. */
function matchImageUrl(match: HnsMatch): string | null {
  const picture = match.homeTeam?.picture ?? match.awayTeam?.picture;
  return picture ? `${BASE_URL}${getCometImageUrl(picture)}` : null;
}
...
export async function generateMetadata({
  ...
  const matchImg = matchImageUrl(match);
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      ...(matchImg
        ? { images: [{ url: matchImg, alt: `${home} – ${away}` }] }
        : {}),
    },
  };
}
